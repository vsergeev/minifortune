...
int filter_extension_dat(const struct dirent *d) {
    /* File name must be at least: "*.dat" */
    size_t len = strlen(d->d_name);
    return (len > 4) && (strncmp(d->d_name + (len - 4), ".dat", 4) == 0);
}
...
int choose_random_datfile(char *dat_path, int maxlen, const char *dir_path) {
    ...
    /* Pick a random .dat file from [0, n-1] */
    unsigned int index = rand() % n;

    /* Assemble the .dat file path */
    snprintf(dat_path, maxlen, "%s/%s", dir_path, fnamelist[index]->d_name);
...
int choose_random_fortune_pos(uint32_t *pos, uint8_t *delim, bool *rot13, const char *dat_path) {
    ...
    /* Pick a random fortune from [0, str_numstr-1] */
    unsigned int index = rand() % dat_header.str_numstr;

    /* Seek to the fortune position in the .dat file positions table */
    if (fseek(fp, index*sizeof(fortune_pos), SEEK_CUR) < 0) {
        fprintf(stderr, "Error seeking to fortune index %d in \"%s\": fseek(): %s\n", index, dat_path, strerror(errno));
        goto cleanup_failure;
    }

    /* Read the fortune position */
    if (fread(&fortune_pos, sizeof(fortune_pos), 1, fp) < 1) {
        fprintf(stderr, "Error reading fortune index %d in \"%s\": fread(): %s\n", index, dat_path, strerror(errno));
        goto cleanup_failure;
    }
...
/* Read a fortune from fortune file fortune_path at position pos with delimiter delim. */
int read_fortune(char **fortune, const char *fortune_path, uint32_t pos, uint8_t delim, bool rot13) {
    ...
    *fortune = NULL;
    ...
    if (fseek(fp, pos, SEEK_SET) < 0) {
        fprintf(stderr, "Error seeking fortune file \"%s\": fseek(): %s\n", fortune_path, strerror(errno));
        goto cleanup_failure;
    }
    ...
    for (len = 0; ; len++) {
        if ((c = fgetc(fp)) == EOF) {
            if (feof(fp))
                fprintf(stderr, "Error reading fortune from \"%s\": End delimiter not found.\n", fortune_path);
            else if (ferror(fp))
    ...
    if (argc == 1) {
        ...
        if (getenv(ENV_FORTUNE_PATH) != NULL && strlen(getenv(ENV_FORTUNE_PATH)) > 0) {
            ...
            if (choose_random_path(path, sizeof(path), getenv(ENV_FORTUNE_PATH)) < 0) {
                fprintf(stderr, "Error, no path found in environment variable paths: %s\n", getenv(ENV_FORTUNE_PATH));
                exit(EXIT_FAILURE);
            }
